...
describe('aggregateCandlesticks', () => {
    ...
    it('opens at the pre-swap price and closes at the post-swap price', () => {
        const events = [makeEvent(1000, true, NATIVE(10), TOKENS(100), 4.4, 3.9)]
        const [candle] = aggregateCandlesticks(events, '1h', 'price')
        expect(candle).toMatchObject({ time: 0, open: 3.9, high: 4.4, low: 3.9, close: 4.4 })
        expect(candle!.volume).toBe(10)
    })

    it('scales price by total supply in mcap mode', () => {
        const events = [makeEvent(1000, true, NATIVE(10), TOKENS(100), 2e-9, 1e-9)]
        const [candle] = aggregateCandlesticks(events, '1h', 'mcap')
        expect(candle!.open).toBeCloseTo(1, 9)
        expect(candle!.close).toBeCloseTo(2, 9)
    })

    it('falls back to the post-swap price when pre-swap price is missing', () => {
        const events = [makeEvent(1000, true, NATIVE(10), TOKENS(100), 4.4, 0)]
        expect(aggregateCandlesticks(events, '1h', 'price')[0]!.open).toBe(4.4)
    })

    it('skips events with no price', () => {
        const events = [makeEvent(1000, true, NATIVE(10), TOKENS(100), 0, 0)]
        expect(aggregateCandlesticks(events, '1h', 'price')).toEqual([])
    })

    it('aggregates events in same time bucket', () => {
        const events = [
            makeEvent(100, true, NATIVE(10), TOKENS(100), 4.4, 3.9),
            makeEvent(200, false, TOKENS(50), NATIVE(5), 4.1, 4.4),
        ]
        const candles = aggregateCandlesticks(events, '1h', 'price')
        expect(candles).toHaveLength(1)
        expect(candles[0]).toMatchObject({ open: 3.9, high: 4.4, low: 3.9, close: 4.1, volume: 15 })
    })

    it('forward-fills missing buckets with the previous close', () => {
        const events = [
            makeEvent(100, true, NATIVE(10), TOKENS(100), 4.4, 3.9),
            makeEvent(7200 + 100, true, NATIVE(10), TOKENS(50), 5, 4.4),
        ]
        const candles = aggregateCandlesticks(events, '1h', 'price')
        expect(candles).toHaveLength(3)
        expect(candles[1]).toMatchObject({ open: 4.4, high: 4.4, low: 4.4, close: 4.4, volume: 0 })
    })

    it('works with different timeframes', () => {
        const events = [makeEvent(100, true, NATIVE(10), TOKENS(100), 4.4, 3.9)]
        expect(aggregateCandlesticks(events, '1m')[0]!.time).toBe(60)
        expect(aggregateCandlesticks(events, '1d')[0]!.time).toBe(0)
    })
})
...
describe('computeFeeBreakdown', () => {
    ...
    it('attributes buy fees to native, sell fees to tokens, and combines both in KUB', () => {
        const events = [
            // buy: 100 KUB in
            makeEvent(100, true, NATIVE(100), TOKENS(1000)),
            // sell: 500 tokens in for 40 KUB out
            makeEvent(200, false, TOKENS(500), NATIVE(40)),
        ]
    ...
    it('charges Durianfun 1.17% in KUB on both sides, sells off the gross proceeds', () => {
        ...
        const events = [
            makeEvent(1, true, 100000000000000000n, 64853481354849072770925n),
            makeEvent(2, false, 64853481354849072770925n, 97679618890000000n),
        ]
